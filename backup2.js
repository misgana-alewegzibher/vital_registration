...
const cpUpload = upload.fields([{ name: 'bUserImg', maxCount: 1 }, { name: 'bUserImg2', maxCount: 1 }]);
// const filesColl = [{ name: 'bUserImg', maxCount: 1 }, { name: 'bUserImg2', maxCount: 1 }];
// const cpUpload = upload.fields(filesColl);

// const cUpload2 = upload.fields(fields,[{ name: 'bUserImg', maxCount: 1 }])
// var cup = upload.files


app.post("/birth", cpUpload, function (req, res) {

  var cimgf = req.files.bUserImg[0].path;
  var cimgb = req.files.bUserImg2[0].path;
  async function getId(){
     return await birth_info.find({}).limit(1).sort({rid:-1}).select({rid:1});
  ...
  const newBirth = new birth_info({
    ...
    // bthid :'bth'+birth_info.findOne({rid:rid}).toString(),
...
app.get("/login/request", function (req, res) {
  if (logged == true) {
    ...
    async function getResult() {
      const bdata = await birth_info.find().sort({ date_of_birthday_registration: 1 }).select({ rid: 1, name: 1, date_of_birthday_registration: 1, userImgF: 1 , userImgB: 1});
      var buf = [];
